import{LitElement,html,svg,css}from"https://unpkg.com/lit@2.0.0/index.js?module";import{live}from"https://unpkg.com/lit@2.0.0/directives/live.js?module";import{unsafeHTML}from"https://unpkg.com/lit@2.0.0/directives/unsafe-html.js?module";const LIB_VERSION="1.0.2";function cgParseNumber(e){const t=String(e).replace(",",".");if("-"===t||"-0"===t||t.endsWith("."))return null;if(t.includes(".")&&t.endsWith("0"))return null;if(""===t)return;const i=parseFloat(t);return isNaN(i)?null:i}function cgTextField(e,t,i,o={}){return html`
  ...
  `;render(){const e=this.options||[];return html`${e.map((t,i)=>{const o=0===i,n=i===e.length-1,a=1===e.length,s=[t.value===this.value?"active":"",a?"only":o?"first":n?"last":""].filter(Boolean).join(" ");return html`<button class="${s}" @click=${()=>this._select(t.value)}>${t.label}</button>`})}`}_select(e){this.value=e,this.dispatchEvent(new CustomEvent("change",{detail:{value:e},bubbles:!0,composed:!0}))}}customElements.define("cg-button-toggle-group",CgButtonToggleGroup);class CgCombobox extends LitElement{static properties={value:{type:String},options:{type:Array},_open:{state:!0},_cursor:{state:!0}};static styles=css`
    :host {
      display: block;
      width: 100%;
      min-width: 0;
        ...
        ${(e.scales||[]).map((e,t)=>html`
          <ha-expansion-panel header="Scale ${t+1}" outlined>
            ...
            <div class="scale-arc-grid">
              ${cgColorPicker("Arc color",e.arc_color||"#333333",e=>this._scaleChanged(t,"arc_color",e))}
              ${cgTextField("Arc width",e.arc_width,e=>this._scaleChanged(t,"arc_width",e),{type:"number",step:"1",min:"0"})}
              ${cgButtonPicker("Linecap",e.arc_linecap,LINECAP_OPTIONS,e=>this._scaleChanged(t,"arc_linecap",e))}
            </div>
              ...
              ${(e.ticks||[]).map((e,i)=>html`
                <ha-expansion-panel header="Tick tier ${i+1}" outlined>
                  ...
                  <div class="tick-type-grid">
                    ...
                    ${cgToggleField("Exclusive",e.exclusive,e=>this._tickChanged(t,i,"exclusive",e))}
  ...
  `}customElements.define("chrono-gauge-card-editor",ChronoGaugeCardEditor);const CARD_VERSION="1.0.23.3";console.info("%c CHRONO-GAUGE-CARD %c v1.0.23.3 ","background-color: #29b6cf; color: #fff; font-weight: bold; padding: 2px 4px; border-radius: 3px 0 0 3px;","background-color: #1e1e1e; color: #fff; font-weight: bold; padding: 2px 4px; border-radius: 0 3px 3px 0;");const GAUGE_DEFAULT_MARGIN=12,GAUGE_BEZEL_RADIUS="50%",DEFAULT_SECTION={value:0,color_start:"#00AA00",color_end:"#00AA00",width:5,position:0,linecap:"butt"},DEFAULT_TICK={show:!0,type:"line",divisions:10,length:3,width:1.5,position:0,color:"#AAAAAA",linecap:"butt",font_size:10,font_weight:400,font_color:"#AAAAAA",exclusive:!0},DEFAULT_NEEDLE={show:!0,template:"{{ states('sensor.temperature') | float(0) }}",color_1:"#FF0000",color_1_pos:50,color_2:"#EEEEEE",color_2_pos:50,width:7,height:40,position:0,morph:40,curve:0,invert:!1,image_show:!1,image_url:"",image_scale:100,image_x:0,image_y:0,image_rotate:0},DEFAULT_FIELD={show:!0,template:"",font_size:1.8,font_weight:400,position:50,font_color:"#FFFFFF",unit:"",unit_font_size:1.4,unit_font_weight:400,unit_font_color:"#FFFFFF"},DEFAULT_SCALE={gap_position:0,gap_size:180,scale_min:0,scale_max:100,arc_rotation:0,position:0,unit:"",clamp_min_offset:0,clamp_max_offset:0,arc_color:"#333333",arc_width:16,arc_linecap:"butt",sections:[],ticks:[],needles:[{...DEFAULT_NEEDLE}]},DEFAULT_CONFIG={gauge_size:100,arc_rotation:0,background_color:"#1a1a1a",bezel_width:3,bezel_color:"#444444",background_image_show:!1,background_image_url:"",background_image_scale:100,background_image_x:0,background_image_y:0,background_image_rotate:0,clip:!1,rotation_animation_time:.5,header_show:!1,header_text:"",header_font_size:1,header_font_weight:400,header_font_color:"#FFFFFF",header_position:0,footer_show:!1,footer_text:"",footer_font_size:1,footer_font_weight:400,footer_font_color:"#FFFFFF",footer_position:0,scales:[{...DEFAULT_SCALE}],fields:[]};function cgGapToArc(e,t){const i=t/2;return{arcStart:(180+e+i+360)%360,arcEnd:(180+e-i+360)%360}}function valueToAngle(e,t,i,o,n){return o+(e-t)/(i-t)*((n-o+360)%360)}function angleToPoint(e,t,i,o){const n=(e-90)*Math.PI/180;return{x:i+t*Math.cos(n),y:o+t*Math.sin(n)}}function buildArcPath(e,t,i,o,n){const a=(t-e+360)%360,s=angleToPoint(e,i,o,n),r=angleToPoint(t,i,o,n),l=a>180?1:0;return`M ${s.x},${s.y} A ${i},${i} 0 ${l} 1 ${r.x},${r.y}`}function buildRingPath(e,t,i,o,n,a){const s=(t-e+360)%360>180?1:0,r=angleToPoint(e,i,n,a),l=angleToPoint(t,i,n,a),c=angleToPoint(t,o,n,a),d=angleToPoint(e,o,n,a);return`M ${r.x},${r.y} A ${i},${i} 0 ${s} 1 ${l.x},${l.y} L ${c.x},${c.y} A ${o},${o} 0 ${s} 0 ${d.x},${d.y} Z`}function cgArcMidpoint(e,t){return(e+(t-e+360)%360/2)%360}function cgScaleBoundingPoints(e,t){const{arcStart:i,arcEnd:o}=cgGapToArc(parseFloat(e.gap_position)||0,parseFloat(e.gap_size)||0),n=(parseFloat(t)||0)+(parseFloat(e.arc_rotation)||0),a=(i+n+360)%360,s=(o+n+360)%360,r=cgArcMidpoint(a,s);return[{x:50,y:50},angleToPoint(a,50,50,50),angleToPoint(r,50,50,50),angleToPoint(s,50,50,50)]}function cgGaugeBoundingBox(e){let t=50,i=50,o=50,n=50;const a=parseFloat(e.arc_rotation)||0;(e.scales||[]).forEach(e=>{cgScaleBoundingPoints(e,a).forEach(e=>{e.x<t&&(t=e.x),e.x>i&&(i=e.x),e.y<o&&(o=e.y),e.y>n&&(n=e.y)})});const s=i-t,r=n-o;return{minX:t,maxX:i,minY:o,maxY:n,aspectRatio:s>0&&r>0?s/r:1}}class ChronoGaugeCard extends LitElement{static properties={_fieldValues:{type:Array},_headerValue:{type:String},_footerValue:{type:String},_backgroundImageUrl:{type:String},_needleValues:{type:Array}};constructor(){super(),this._config=null,this._hass=null,this._templateUnsubs=[],this._subscriptionsActive=!1,this._fieldValues=[],this._headerValue="",this._footerValue="",this._backgroundImageUrl="",this._needleValues=[]}set hass(e){this._hass=e,this._config&&!this._subscriptionsActive&&this._setupSubscriptions()}get hass(){return this._hass}setConfig(e){this._config={...DEFAULT_CONFIG,...e},this._config.scales=(e.scales||[{...DEFAULT_SCALE}]).map(e=>({...DEFAULT_SCALE,...e,sections:(e.sections||[]).map(e=>({...DEFAULT_SECTION,...e})),ticks:(e.ticks||[]).map(e=>({...DEFAULT_TICK,...e})),needles:(e.needles||[]).map(e=>({...DEFAULT_NEEDLE,...e}))})),this._config.fields=(e.fields||[]).map(e=>({...DEFAULT_FIELD,...e}));const t=this._config,i=112-(parseFloat(t.gauge_size)??100),{aspectRatio:o}=cgGaugeBoundingBox(this._config);this.style.setProperty("--cg-gauge-margin",`${i}%`),this.style.setProperty("--cg-gauge-aspect-ratio",`${o}`),this.style.setProperty("--cg-bezel-width",parseFloat(t.bezel_width)/2+"cqi"),this.style.setProperty("--cg-bezel-color",t.bezel_color),this.style.setProperty("--cg-background-color",t.background_color),this.style.setProperty("--cg-bezel-radius","50%"),this.style.setProperty("--cg-animation-duration",`${t.rotation_animation_time}s`),t.card_height&&(this.style.height=t.card_height),this._hass&&!this._subscriptionsActive&&this._setupSubscriptions()}get config(){return this._config}connectedCallback(){super.connectedCallback(),this._hass&&this._config&&this._setupSubscriptions()}disconnectedCallback(){super.disconnectedCallback(),this._teardownSubscriptions()}_setupSubscriptions(){if(this._teardownSubscriptions(),!this.hass?.connection||!this.config)return;this._subscriptionsActive=!0;const e=(e,t)=>{const i=String(e);if(!i.includes("{{"))return void t(i);const o=this.hass.connection.subscribeMessage(e=>t(e.result),{type:"render_template",template:i});this._templateUnsubs.push(o)};(this.config.fields||[]).forEach((t,i)=>{t.show?e(String(t.template),e=>{const t=[...this._fieldValues];t[i]=String(e),this._fieldValues=t}):this._fieldValues[i]=""}),this.config.header_show&&this.config.header_text?e(this.config.header_text,e=>{this._headerValue=e}):this._headerValue="",this.config.footer_show&&this.config.footer_text?e(this.config.footer_text,e=>{this._footerValue=e}):this._footerValue="",this.config.background_image_show?e(this.config.background_image_url,e=>{this._backgroundImageUrl=e}):this._backgroundImageUrl="",this._needleValues=[],(this.config.scales||[]).forEach((t,i)=>{this._needleValues[i]||(this._needleValues[i]=[]),(t.needles||[]).forEach((t,o)=>{t.show?e(t.template,e=>{const t=parseFloat(e),n=this._needleValues.map(e=>[...e]);n[i][o]=isNaN(t)?void 0:t,this._needleValues=n,this.requestUpdate()}):this._needleValues[i][o]=void 0})})}_teardownSubscriptions(){const e=this._templateUnsubs;this._templateUnsubs=[],this._subscriptionsActive=!1;for(const t of e)Promise.resolve(t).then(e=>e().catch(()=>{})).catch(()=>{})}_renderScaleArc(e,t){const i=parseFloat(e.arc_width)??16,o=50+(parseFloat(e.position)??0)-i/2,{arcStart:n,arcEnd:a}=cgGapToArc(parseFloat(e.gap_position)??0,parseFloat(e.gap_size)??180),s=buildArcPath(n,a,o,50,50);return html`
      <div class="gauge-scale-layer">
        <svg class="gauge-scale-svg" viewBox="0 0 100 100" preserveAspectRatio="none">
          <path
            d="${s}"
            fill="none"
            stroke="${e.arc_color||"#333333"}"
            stroke-width="${i}"
            stroke-linecap="${e.arc_linecap||"butt"}"
          />
        </svg>
      </div>
    `}_renderScaleSections(e,t){const i=e.sections||[];if(0===i.length)return html``;const o=parseFloat(e.scale_min)||0,n=parseFloat(e.scale_max)||100,{arcStart:a,arcEnd:s}=cgGapToArc(parseFloat(e.gap_position)||0,parseFloat(e.gap_size)||180),r=[...i].sort((e,t)=>parseFloat(e.value)-parseFloat(t.value));return html`
      <div class="gauge-scale-layer">
        <svg class="gauge-scale-svg" viewBox="0 0 100 100" preserveAspectRatio="none" overflow="visible">
          <defs>
            ${r.map((e,i)=>{const l=parseFloat(e.value),c=i<r.length-1?parseFloat(r[i+1].value):n,d=valueToAngle(l,o,n,a,s),g=valueToAngle(c,o,n,a,s),p=parseFloat(e.width)||16,h=50+(parseFloat(e.position)||0),u=buildRingPath(d,g,h,h-p,50,50);return svg`
                <clipPath id="${`section-clip-${t}-${i}`}" clipPathUnits="userSpaceOnUse">
                  <path d="${u}" />
                </clipPath>
              `})}
          </defs>

          ${r.map((e,i)=>{const l=parseFloat(e.value),c=i<r.length-1?parseFloat(r[i+1].value):n,d=valueToAngle(l,o,n,a,s),g=valueToAngle(c,o,n,a,s),p=parseFloat(e.width)||16,h=parseFloat(e.position)||0,u=`section-clip-${t}-${i}`,m=e.color_start||"#ffffff",f=e.color_end||m;if(m===f){const e=50+h,t=buildRingPath(d,g,e,e-p,50,50);return svg`<path d="${t}" fill="${m}" />`}const _=d,b=(g-d+360)%360,x=50..toFixed(4);return svg`
              <foreignObject x="${-12}" y="${-12}" width="${124}" height="${124}" clip-path="url(#${u})">
                <div
                  xmlns="http://www.w3.org/1999/xhtml"
                  style="width:100%;height:100%;background:conic-gradient(from ${_}deg at ${x}% ${x}%, ${m} 0deg, ${f} ${b}deg, transparent ${b}deg);"
                ></div>
              </foreignObject>
            `})}
        </svg>
      </div>
    `}_renderScaleTicks(e,t){const i=e.ticks||[];if(0===i.length)return html``;const o=50,n=parseFloat(e.scale_min)||0,a=parseFloat(e.scale_max)||100,{arcStart:s,arcEnd:r}=cgGapToArc(parseFloat(e.gap_position)||0,parseFloat(e.gap_size)||180),l=e=>Math.round(1e4*e)/1e4,c=new Set,d=[];return i.forEach(e=>{if(!e.show)return;const t=parseInt(e.divisions)||1,i=!1!==e.exclusive,g=e.type||"line",p=parseFloat(e.position)||0;for(let h=0;h<=t;h++){const u=n+h/t*(a-n),m=l(valueToAngle(u,n,a,s,r));if(c.has(m))continue;i&&c.add(m);const f=(m-90)*Math.PI/180,_=Math.cos(f),b=Math.sin(f);if("line"===g){const t=Math.max(parseFloat(e.length),.001),i=50+(o+p)*_,n=50+(o+p)*b,a=50+(o+p-t)*_,s=50+(o+p-t)*b;d.push(svg`
            <line
              x1="${i}" y1="${n}" x2="${a}" y2="${s}"
              stroke="${e.color||"#AAAAAA"}"
              stroke-width="${parseFloat(e.width)||1.5}"
              stroke-linecap="${e.linecap||"butt"}"
            />
          `)}else if("number"===g){const t=parseFloat(e.font_size)||10,i=parseFloat(e.font_weight)||400,n=e.font_color||"#AAAAAA",a=50+(o+p)*_,s=50+(o+p)*b,r=Number.isInteger(u)?u:parseFloat(u.toFixed(2));d.push(svg`
            <text
              x="${a}" y="${s}"
              text-anchor="middle"
              ...
              fill="${n}"
            >${r}</text>
          `)}}}),0===d.length?html``:html`
      <div class="gauge-scale-layer">
        <svg class="gauge-scale-svg" viewBox="0 0 100 100" preserveAspectRatio="none" overflow="visible">
          ${d}
        </svg>
      </div>
    `}_buildNeedlePath(e,t,i,o,n,a){const s=n/2;let r=[{x:50,y:0},{x:50-s,y:a},{x:50,y:a+e},{x:50+s,y:a}];const l=t/50*.5523,c=l*s,d=l*a,g=l*e;let p=[{in:{x:1,y:0},out:{x:-1,y:0},inDist:c,outDist:c},{in:{x:0,y:-1},out:{x:0,y:1},inDist:d,outDist:g},{in:{x:-1,y:0},out:{x:1,y:0},inDist:c,outDist:c},{in:{x:0,y:1},out:{x:0,y:-1},inDist:g,outDist:d}];i&&(r.forEach(e=>{e.y=a-e.y}),p.forEach(e=>{e.in.y=-e.in.y,e.out.y=-e.out.y})),r.forEach(e=>{e.y+=50-a-o});const h=r.map((e,t)=>({in:{x:e.x+p[t].in.x*p[t].inDist,y:e.y+p[t].in.y*p[t].inDist},out:{x:e.x+p[t].out.x*p[t].outDist,y:e.y+p[t].out.y*p[t].outDist}}));let u=`M ${r[0].x},${r[0].y}`;for(let e=0;e<r.length;e++){const t=(e+1)%r.length;u+=` C ${h[e].out.x},${h[e].out.y} ${h[t].in.x},${h[t].in.y} ${r[t].x},${r[t].y}`}return u+=" Z",u}_renderScaleOverlay(e,t){const i=parseFloat(e.scale_min)??0,o=parseFloat(e.scale_max)??100,n=i-(parseFloat(e.clamp_min_offset)??0),a=o+(parseFloat(e.clamp_max_offset)??0),{arcStart:s,arcEnd:r}=cgGapToArc(parseFloat(e.gap_position)??0,parseFloat(e.gap_size)??180);return html`
      ${(e.needles||[]).map((e,l)=>{if(!e.show)return html``;const c=this._needleValues?.[t]?.[l];if(void 0===c)return html``;const d=valueToAngle(Math.min(Math.max(c,n),a),i,o,s,r),g=this._buildNeedlePath(parseFloat(e.morph),parseFloat(e.curve),e.invert,parseFloat(e.position),parseFloat(e.width),parseFloat(e.height)),p=`needleGradient-${t}-${l}`,h=e.invert?e.color_2:e.color_1,u=e.invert?100-parseFloat(e.color_2_pos):parseFloat(e.color_1_pos),m=e.invert?e.color_1:e.color_2,f=e.invert?100-parseFloat(e.color_1_pos):parseFloat(e.color_2_pos);return html`
          <div class="gauge-needle-layer" style="transform:rotate(${d}deg)">
            <svg class="gauge-needle-svg"
              ...
              <defs>
                <linearGradient id="${p}" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <stop offset="${u}%" stop-color="${h}" />
                  <stop offset="${f}%" stop-color="${m}" />
      ...
      <ha-card>
        <div class="gauge-container">
          ...
          <div class="gauge-layer">
            ...
            ${(e.scales||[]).map((e,t)=>html`
              ${this._renderScaleArc(e,t)}
              ${this._renderScaleSections(e,t)}
              ${this._renderScaleTicks(e,t)}
            `)}
    ...
    .gauge-container {
      ...
      height: 100%;
      min-height: 100%;
      aspect-ratio: var(--cg-gauge-aspect-ratio);
      margin: 0 auto;
      overflow: hidden;
      box-sizing: border-box;
